...
def process():
    ...
    out_name = "Loans_and_Interest.xlsx"          # on-disk name (safe)
    ...
    # Friendly filename derived from the report period, e.g.
    # "April 2026 Loans & Interest.xlsx".
    friendly = info.get("download_name") or "Loans & Interest.xlsx"
    info["download_url"] = (
        f"/download/{job}/{out_name}?name={quote(friendly)}")
    return jsonify(info)
...
def download(job, name):
    ...
    if not os.path.isfile(path):
        abort(404)
    # Use the friendly name from the query string for the saved file.
    friendly = request.args.get("name") or safe
    friendly = os.path.basename(friendly)        # strip any path components
    if not friendly.lower().endswith((".xlsx", ".xlsm")):
        friendly += ".xlsx"
    return send_file(path, as_attachment=True, download_name=friendly)
