...
# loan type -> {tab, group}
LOAN_TYPES = {
    "affiliates":  {"tab": "YS Affiliates > Affiliates",  "group": "affiliates"},
    "ystix":       {"tab": "YS Tix > YS Affiliates",      "group": "ystix"},
    "damona":      {"tab": "Mazel > Damona",              "group": "mazel"},
    "eitz_chaim":  {"tab": "Eitz Chaim, TV > Mazel",      "group": "mazel"},
}

# ----- styling (neutral; no yellow highlight, no blue font) --------------
FONT_NAME = "Arial"
SUMMARY_FONT = "Calibri"          # body font on the Loan Terms summary tab
DARK_BLUE = "1F4E79"              # highlight color (replaces gray)
WHITE = "FFFFFF"
HDR_FILL = PatternFill("solid", start_color=DARK_BLUE)
ACCT_FILL = PatternFill("solid", start_color=DARK_BLUE)
TOTAL_FILL = PatternFill("solid", start_color=DARK_BLUE)
GRAND_FILL = PatternFill("solid", start_color=DARK_BLUE)
...
# Known company-name aliases to normalize on screen and in output files.
COMPANY_ALIASES = {
    "mazel 2": "Mazel Investing",
}
...
def _build_loan_terms(wb, company, rates, days_in_year):
    ...
    body = Font(name=SUMMARY_FONT, size=11)
    ...
    for blk in LOAN_TERM_BLOCKS:
        ...
        a = ws.cell(row=r, column=1, value=blk["lender_label"]); a.font = body
        b = ws.cell(row=r, column=2, value=blk["lender"]); b.font = body; b.alignment = left
        a = ws.cell(row=r + 1, column=1, value=blk["borrower_label"]); a.font = body
        b = ws.cell(row=r + 1, column=2, value=blk["borrower"]); b.font = body; b.alignment = left
        a = ws.cell(row=r + 2, column=1, value="Terms"); a.font = body

        rate_row, days_row, daily_row = r + 3, r + 4, r + 5
        # The three "Terms" detail rows are indented one level under "Terms".
        ar = ws.cell(row=rate_row, column=1, value="Annual Interest Rate")
        ar.font = body; ar.alignment = left_indent
        br = ws.cell(row=rate_row, column=2, value=rate)
        br.font = body; br.alignment = left; br.number_format = "0.000%"
        ad = ws.cell(row=days_row, column=1, value="# Days in Year")
        ad.font = body; ad.alignment = left_indent
        bd = ws.cell(row=days_row, column=2, value=days_in_year)
        bd.font = body; bd.alignment = left; bd.number_format = "0"
        ai = ws.cell(row=daily_row, column=1, value="Daily Interest Rate")
        ai.font = body; ai.alignment = left_indent
        bi = ws.cell(row=daily_row, column=2, value=f"=B{rate_row}/B{days_row}")
        bi.font = body; bi.alignment = left; bi.number_format = "0.00000000%"

        note_row = r + 6
        an = ws.cell(row=note_row, column=1, value="QBO Notes"); an.font = body
        for i, note in enumerate(blk["notes"]):
            bn = ws.cell(row=note_row + i, column=2, value=note)
            bn.font = body; bn.alignment = left
    ...
    ws.column_dimensions["A"].width = 23.43
    ws.column_dimensions["B"].width = 17.43
    ws.column_dimensions["C"].width = 36.43
    ws.column_dimensions["D"].width = 72.0
    return daily_cells


# ----- Calculation sheet -------------------------------------------------
def _autosize_columns(ws, min_col=1, max_col=15):
    """
    Widen every column so its content (and the dark-blue highlight) is never
    clipped. Formula cells are sized by an estimate of their displayed value
    rather than the formula text, money cells include $ and separators, and
    the header row (which wraps) is not allowed to force huge widths.
    """
    hdr_row = 5
    widths = {}
    for row in ws.iter_rows(min_col=min_col, max_col=max_col):
        for cell in row:
            ...
            if isinstance(v, str) and v.startswith("="):
                ...
                text = "$9,999,999.99" if "$" in fmt else "9,999"
                ...
                text = str(v)
            if cell.row == hdr_row and isinstance(v, str):
                longest = max((len(w) for w in v.split()), default=len(v))
                length = longest + 2
            else:
                length = len(text)
    ...
    for col, length in widths.items():
        ws.column_dimensions[col].width = max(9, min(length + 2, 60))
...
def _build_calc_sheet(wb, parsed, tab_name, daily_cell, sign):
    ...
    for i, h in enumerate(headers):
        ...
        c.font = Font(name=FONT_NAME, bold=True, color=WHITE)
        c.fill = HDR_FILL
    ...
    for blk in parsed["blocks"]:
        ...
        lbl.font = Font(name=FONT_NAME, bold=True, color=WHITE)
        lbl.fill = ACCT_FILL
        row += 1
        block_start = row
        ...
        ws.cell(row=tot, column=7, value=f"Total — {blk['account']}").font = \
            Font(name=FONT_NAME, bold=True, color=WHITE)
        ...
        for cc in (12, 13):
            ws.cell(row=tot, column=cc).font = Font(name=FONT_NAME, bold=True, color=WHITE)
    ...
    ws.cell(row=g, column=7, value="TOTAL MONTHLY INTEREST").font = \
        Font(name=FONT_NAME, bold=True, size=11, color=WHITE)
    gt = ws.cell(row=g, column=13,
                 value=("=" + "+".join(sum_cells)) if sum_cells else 0)
    gt.number_format = MONEY
    gt.font = Font(name=FONT_NAME, bold=True, size=11, color=WHITE)
    gt.fill = GRAND_FILL
...
def generate(jobs, output_path, rates=None, days_in_year=365):
    ...
    # Determine the report period (month + year) from the parsed contents so
    # the download can be named e.g. "April 2026 Loans & Interest.xlsx".
    months = [p["parsed"]["month"] for p in prepared if p["parsed"]["month"]]
    years = [p["parsed"]["year"] for p in prepared if p["parsed"]["year"]]
    month = months[0] if months else None
    year = years[0] if years else None
    if month and year:
        period_label = f"{calendar.month_name[month]} {year}"
        download_name = f"{calendar.month_name[month]} {year} Loans & Interest.xlsx"
    else:
        period_label = ""
        download_name = "Loans & Interest.xlsx"

    return {"company": company, "tabs": tabs,
            "month": month, "year": year,
            "period_label": period_label, "download_name": download_name}
